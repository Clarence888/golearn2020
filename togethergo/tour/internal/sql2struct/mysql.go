...
func (m *DBModel) Connect() error  {
	var err error
	var link = "%s:%s@tcp(%s)/information_schema?" + "charset=%s&parseTime=True&loc=Local"
	dsn := fmt.Sprintf(
		link,
		m.DBInfo.UserName,
		m.DBInfo.Password,
	...
	m.DBEngine,err = sql.Open(m.DBInfo.DBType,dsn)// 第一个为驱动名称 比如mysql  第二个为连接信息
	if err != nil {
		return err
	}
